...
m3.metric("Uptime", f"{int(time.time() - manager.buffer._buffer[0]['timestamp']) if status.is_active and status.buffer_size > 0 else 0}s")
...
def main():
    import os
    import sys
    # This function is the entry point for the monitor-mcp-ui script
    # It just runs streamlit on itself
