...
def show_ui():
    ...
    with st.sidebar.expander("Monitoring", expanded=not is_simulating):
        ...
        sel_mon = st.selectbox("Screen", options=opt_list, index=0, key="sel_mon")
        screen_idx = monitor_options[sel_mon]
        freq = st.slider("Hz", 0.1, 30.0, float(defaults.frequency), key="freq")
        m_imgs = st.number_input("Max Imgs", 1, 10000, int(defaults.max_images), key="m_imgs")
        s_disk = st.checkbox("Save Disk", value=defaults.save_to_disk, key="s_disk")
        r_cache = st.checkbox("Reset Cache", value=defaults.reset_cache, key="r_cache")
        d_mouse = st.checkbox("Draw Mouse", value=defaults.draw_mouse, key="d_mouse")
        path = st.text_input("Path", value=defaults.storage_path, key="path")
        ttl = st.number_input("TTL (min)", 0, 1440, int(defaults.ttl_minutes), help="Auto-stop after X minutes. 0 for no limit.", key="ttl")
    ...
    with st.sidebar.expander("AI Sandbox", expanded=is_simulating):
        sim_folder = st.text_input("Folder", value="E:\\test_recording", key="sim_folder")
        sim_model = st.selectbox("Model", ["gemini-3.1-flash-lite-preview", "gemini-2.0-flash-lite-preview-02-05"], key="sim_model")
        sim_prompt = st.text_area("Prompt", value="Describe the screen.", key="sim_prompt")
        sim_delay_in_seconds = st.number_input("Delay (s)", 5, 3600, 15, key="sim_delay")
        sim_frame_count = st.number_input("Count (frames)", 1, 20, 9, key="sim_cnt")
        sim_frame_interval = st.number_input("Interval (step)", -100, 100, -10, key="sim_int")
        sim_frame_offset = st.number_input("Offset (frame index)", -10000, 10000, -1, key="sim_off")
        sim_ttl = st.number_input("TTL (min)", 0, 1440, 0, help="Auto-stop after X minutes. 0 for no limit.", key="sim_ttl")
    ...
    with st.sidebar.expander("Manual Query", expanded=False):
        q_start_frame_index = st.number_input("Start Frame Index", value=-1, key="q_start")
        q_frame_count = st.number_input("Frame Count", 1, 100, 12, key="q_count")
        q_frame_interval = st.number_input("Frame Interval", -100, 100, -1, key="q_interval")
        ...
        if not session_names:
            ...
            with st.container(height=600, border=True):
                selected_session_name = st.radio(
                    ...
                    index=session_names.index(st.session_state.selected_session_name),
